...
def seed_if_empty(db: Session) -> None:
    ...
    db.add(store)
    db.flush()
    r1 = HangRail(store_id=store.id, label="A 杆", length_cm=200, max_garment_cm=80)
    ...
    orders = [
        ...
        WorkOrder(store_id=store.id, ticket_code="HR-2003", garment_name="羽绒服", length_cm=90, status="ready", due_at=now + timedelta(days=1)),
